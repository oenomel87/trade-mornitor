...
설정 파일 v1의 주요 키와 내장 기본값은 다음과 같다. 생략 키에는 기본값을 적용한다. `model=null`은 SDK의 설정된 기본 모델을 사용하되 실행 기록에는 실제 해석된 모델을 남긴다는 뜻이다.
...
- SDK 0.147.0을 선택적 의존성으로 고정했다. 포함 런타임에서 기존 ChatGPT 로그인을 확인하며 API 키 계정이면 조사를 실행하지 않는다. 실제 기본 모델을 조회해 사용하고 추론 노력은 low다.
...
- 구조화 출력 실패는 자동 재시도 없이 정량 결과로 복귀한다. 검색 이벤트가 한 번도 없으면 웹 조사 성공으로 취급하지 않는다. 기본 모델 선택으로 만든 캐시는 선택값 `null`을 키에 넣고 실제 사용 모델도 저장한다. 캐시 기간 중 기본 모델 변경은 다음 갱신에 반영된다.
